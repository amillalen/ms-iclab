...
pipeline {
    ...
    stages {
        ...
        stage('update version and tag') {
           when{ expression{ is_master_branch } }
           steps{
              sh "./mvnw -B -Darguments=\"-Dmaven.test.skip=true\" -Dscm.developerConnection=scm:git:git@github.com:amillalen/ms-iclab.git -Dresume=false release:prepare release:perform"
           }        
        }
        stage('nexus') {
           when{ expression{ is_master_branch } }
